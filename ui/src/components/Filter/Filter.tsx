  ...
  const handleTagToggle = (tag: string) => {
    ...
    if (tag === 'All') {
      ...
      setSelectedTags((prevTags) => {
        const updatedTags = prevTags.includes('All')
          ? [tag]
          : prevTags.includes(tag)
          ? prevTags
          : [...prevTags, tag];

        const anySelectedTagsIncludedInRemaining = updatedTags.some((tag) =>
          remainingTags.some((remainingTag) => remainingTag.tag === tag)
        );
      ...
      >
        ...
        >
          <Stack direction="row" sx={{ gap: '4px', flexWrap: 'wrap' }}>
            {tagsWithMostPost &&
              [...[{ tag: 'All', id: 0 }], ...tagsWithMostPost].map(
                (option: { tag: string; id: number }, index: number) => (
                  <Tags
                    name={option.tag}
                    key={index}
      ...
      <Box>
        ...
        >
          {activeAction === 'Others' ? (
            ...
            <CreateTagDropdown onClose={() => setAnchorEl(null)} />
          )}
        </Container>
      </Box>
    </>
